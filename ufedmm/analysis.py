...
class FreeEnergyAnalyzer(object):
    ...
    ----------
        ...
        dataframe : pandas.DataFrame
            A data frame containing sampled sets of collective variables and driver
            parameters.
    """
    ...
    def __init__(self, ufed, dataframe):
        ...
        self._bias_variables = filter(
            lambda v: v.sigma is not None, self._ufed.variables
        )

    def metadynamics_bias_free_energy(self):
        """Returns a Python function which, in turn, receives the values of extended-
        space variables and returns the energy estimated from a Metadynamics bias
        potential reconstructed from the simulation data.
        ...
        variables = [
            Variable(
                v.sigma, 2 * np.pi / v._range, v.periodic, self._dataframe[v.id].values
            )
    ...
    def centers_and_mean_forces(self, bins, min_count=1, adjust_centers=False):
        ...
        ----------
            bins : list(int) or int
                The number of bins in each direction. If a single integer is passed,
                then the same number of bins will be considered for all directions.
        ...
        ------------
            ...
            adjust_centers : bool, default=False
                Whether to consider the center of a bin as the mean value of the its
                sampled internal points instead of its geometric center.

        Returns
        -------
            centers : list(numpy.array)
                A list of Numpy arrays, each one containing the values of an
                extended-space variable at the centers of all bins that satisfy the
                minimum-count criterion.
            mean_forces : list(numpy.array)
                A list of Numpy arrays, each one containing the mean forces in the
                direction of an extended-space variable.
        """
        ...
        counts = stats.binned_statistic_dd(
            sample, [], statistic="count", bins=bins, range=ranges
        )
        index = np.where(counts.statistic.flatten() >= min_count)

        n = len(variables)
        if adjust_centers:
            means = stats.binned_statistic_dd(
                sample, sample + forces, bins=bins, range=ranges
            )
    ...
    def mean_force_free_energy(
        ...
        """Returns Python functions for evaluating the potential of mean force and their
        originating mean forces as a function of the collective variables.
        ...
        ------------
            platform_name : string, default='Reference'
                The name of the OpenMM Platform to be used for potential and mean-force
                evaluations.
        ...
        -------
            potential : function
                A Python function whose arguments are collective variable values and
                whose result is the potential of mean force at that values.
            mean_force : function
                A Python function whose arguments are collective variable values and
                whose result is the mean force at those values.
        """
        variables = self._ufed.variables
        n = len(variables)
        try:
        ...
        for v, variance in zip(variables, variances):
            if v.periodic:  # von Mises
                factor = 2 * np.pi / v._range
                exponent.append(
                    lambda x: (np.cos(factor * x) - 1.0) / (factor * factor * variance)
                )
        ...
        context = _RBFContext(
            variables, variances, grid_points, A, platform, properties
        )
        minimum = 0.0
...
class Analyzer(FreeEnergyAnalyzer):
    ...
    .. warning::
        This class is obsolete and will be discontinued. Use :class:`FreeEnergyAnalyzer`
        instead.
    ...
    def free_energy_functions(self, sigma=None, factor=8):
        ...
        ------------
            sigma : float or unit.Quantity, default=None
                The standard deviation of kernels. If this is `None`, then values will
                be determined from the distances between nodes.
        ...
        -------
            ...
            mean_force : function
                A Python function whose arguments are collective variable values and
                whose result is the mean force at that values regarding a given
                direction. Such direction must be defined through a keyword argument
                `dir`, whose default value is `0` (meaning the direction of the first
                collective variable).
        """
